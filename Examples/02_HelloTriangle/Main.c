...
int main() {
  ...
  while (!concorde_should_close()) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
      concorde_immediate_vertex(cvm_position, triangle_vert_pos[i][0],
                                triangle_vert_pos[i][1],
                                triangle_vert_pos[i][2]);
      concorde_immediate_vertex(cvm_color, triangle_vert_col[i][0],
                                triangle_vert_col[i][1],
